...
$stmtMessage = $db->prepare("SELECT * FROM (SELECT * FROM ".DBTBL.".chats_messages WHERE chat = :chat AND id > :last ORDER BY id DESC LIMIT 255) AS last ORDER BY id ASC; ");
...
while($m = $stmtMessage->fetchObject()){
	...
	$messages[] = array('text' => $m['text'], 'type' => $m['type'], 'time' => $m['time'], 'id' => $m['id'], 'chat' => $chat_data['id'], 'author' => (array)$stmtAuthor->fetchObject(), 'new' => ($m['id'] > $old_last_id));
}
if($member){
